...
pub unsafe fn syscall(cptr: usize, method: usize) -> usize {
    ...
    if let Some(ref mut stream) = *global {
        let msg = HostedMessage::SysInvoke { 
            cptr, 
            method,
            utcb_ptr: UTCB_VA,
        };
        ...
        // 写入长度和消息
        if stream.write_all(&len.to_le_bytes()).is_err() { return usize::MAX; }
        if stream.write_all(&bytes).is_err() { return usize::MAX; }
        if stream.flush().is_err() { return usize::MAX; }
        ...
        eprintln!("[GLENDA-HOSTED] Syscall failed: Not connected to hutch (cptr: 0x{:x}, method: {})", cptr, method);
        usize::MAX
    }
}
